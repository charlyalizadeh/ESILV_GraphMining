...
# Monopartite
# ====NODE====
# Article Name, Subjects (delimited by a `|` categorypaths and a `.` for the categories), ID (to create)
# ====LINK===
# ID_from, ID_to, r:WIKILINK
# ====PATH====
# * add relationship using the distance matrix (maybe too long, 4616 node, so 4616 * 4615 link = 21 302 840)
...
def build_mono_partite_nodes(return_article2id=True):
    ...
    with open('data/wikispeedia_paths-and-graph/articles.tsv', 'r') as article_file:
        for i, article in enumerate(article_file.readlines()[12:]):
            article = article.strip()
            rows['id'].append(i)
            rows['article'].append(article)
            rows['article_decoded'].append(unquote(article))
            cat = '|'.join(categories[article])
            cat = cat if not all([c == ' ' for c in cat]) else "NaN"
            rows['categories'].append(cat)

    df = pd.DataFrame(data=rows)
    df.to_csv('data/mono_partite/nodes.csv', index=False, sep=',')
    if return_article2id:
        return dict(zip(rows['article'], rows['id']))
...
def build_bi_partite_graph():
    ...
    df_articles.to_csv('data/bi_partite/articles.csv', index=False, sep=',')
    df_categories.to_csv('data/bi_partite/categories.csv', index=False, sep=',')
    df_relationships.to_csv('data/bi_partite/relationships.csv', index=False, sep=',')


article2id = build_mono_partite_nodes()
build_mono_partite_relationship(article2id)
build_bi_partite_graph()
